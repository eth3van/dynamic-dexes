...
contract MultiswapRouterComponent is BaseOwnableComponent, IMultiswapRouterComponent {
    ...
    /// @inheritdoc IMultiswapRouterComponent
    function multiswap2(IMultiswapRouterComponent.Multiswap2Calldata calldata data) external returns (uint256) {
        ...
        if (length != data.amountInPercentages.length) {
            revert IMultiswapRouterComponent.MultiswapRouterComponent_InvalidMultiswap2Calldata();
        }
        ...
        // scope for transfer, avoids stack too deep errors
        {
            ...
            if (token == address(0) && amount == 0) {
                ...
                TransferHelper.safeTransferFrom({
                    token: tokenIn,
                    from: TransientStorageComponentLibrary.getSenderAddress(),
    ...
    function _wrapNative(address tokenIn, uint256 amount) internal returns (address _tokenIn) {
        if (tokenIn == address(0)) {
            ...
            TransientStorageComponentLibrary.setTokenAndAmount({ token: address(_wrappedNative), amount: amount });
