...
/// @title ILayerZeroComponent - LayerZeroComponent Interface
interface ILayerZeroComponent {
    // =========================
    // errors
    // =========================

    /// @notice Throws when passed array length does not match
    error LayerZeroComponent_LengthMismatch();

    /// @notice Throws when native amount on contract does not gte fee amount
    error LayerZeroComponent_FeeNotMet();
    ...
    /// @notice Returns fee estimate in LayerZero protocol for passed `dstEid` and `nativeAmount`.
