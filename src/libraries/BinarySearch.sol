...
library BinarySearch {
    ...
    function binarySearch(bytes4 selector, bytes memory componentsAndSelectors) internal pure returns (address component) {
        ...
        assembly ("memory-safe") {
            ...
            for {
                ...
                let offset := add(componentsAndSelectors, 32)
                let low
                let high := div(mload(componentsAndSelectors), 24)
                let mid
                let midValue
                let midSelector
            } lt(low, high) { } {
                ...
                if eq(midSelector, selector) {
                    component := and(shr(64, midValue), addressMask)
                    break
                }
