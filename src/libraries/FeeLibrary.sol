...
import { TransferHelper } from "../components/libraries/TransferHelper.sol";
import { TransientStorageComponentLibrary } from "./TransientStorageComponentLibrary.sol";
...
library FeeLibrary {
    ...
    function payFee(address token, uint256 amount) internal returns (uint256) {
        if (!TransientStorageComponentLibrary.isFeePaid()) {
