...
import { StargateComponent, IStargateComposer } from "../../src/components/bridges/StargateComponent.sol";

import { DeployEngine } from "../DeployEngine.sol";

contract TestnetTestTxs is Script {
    Factory proxy = Factory(payable(0x29F4Bf32E90cAcb299fC82569670f670d334630a));

    // ===================
    // helpers for stargateComponent sends
    // ===================
    ...
    function run() external {
        ...
        (uint256 fee,) = StargateComponent(address(proxy)).quoteV2(stargatePool, dstEidV2, 10e6, deployer, bytes(""), 0);

        proxy.multicall{ value: fee }(
            Solarray.bytess(
                abi.encodeCall(StargateComponent.sendStargateV2, (stargatePool, dstEidV2, 10e6, deployer, 0, bytes("")))
            )
        );
