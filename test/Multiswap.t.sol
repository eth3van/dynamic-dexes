...
contract MultiswapTest is Test {
    ...
    function setUp() external {
        ...
        selectors[0] = IMultiswapRouterComponent.multiswap.selector;
        selectors[1] = IMultiswapRouterComponent.partswap.selector;
        selectors[2] = IMultiswapRouterComponent.wrappedNative.selector;
        selectors[3] = IMultiswapRouterComponent.feeContract.selector;
        selectors[4] = IMultiswapRouterComponent.setFeeContract.selector;
        address[] memory components = Solarray.addresses(
        ...
        InitialImplementation(address(router)).upgradeTo(
            address(new Factory(DeployEngine.getBytesArray(selectors, components))),
            abi.encodeCall(Factory.initialize, (owner, initData))
        );
        vm.stopPrank();
    }

    function test_multiswapRouter_multiswap_swapViaOnePairV3() external {
        IMultiswapRouterComponent.MultiswapCalldata memory data;
