...
contract SymbiosisComponentTest is BaseTest {
    ...
    function test_symboisisComponent_send() external {
        _resetPrank(user);

        IERC20(USDC).transfer({ to: address(factory), amount: 1000e18 });
        ...
        factory.multicall({
            data: Solarray.bytess(
                abi.encodeCall(
                    SymbiosisComponent.send,
                    (
                        SymbiosisComponent.SymbiosisTransaction({
